...
→ A founder sees that LLMs can summarise documents.
→ They build a document summarisation tool.
→ The summaries are good. The technology works.
→ Nobody uses it.
...
→ Edge cases that represent 30% of real-world usage
→ Adversarial inputs (users testing the AI’s limits, which they always do)
...
→ “Did the user get the right answer?” (not “did the model generate text?”)
→ “Did the AI save time compared to the manual process?” (not “did the AI produce output?”)
→ “Did the user come back the next day?” (not “did the user say they liked it?”)
...
→ Can the user easily identify when the AI is wrong?
→ Can the user correct the AI or override it?
→ Does the system degrade gracefully (giving a partial answer rather than a confidently wrong one)?
→ Are wrong outputs captured for future improvement?
...
*If you’re building an AI product and want to avoid these patterns, [start with a conversation](https://calendar.app.google/Nzw5hcFznoxmFEra6). We’ll tell you honestly whether your idea has the structural foundations to succeed, and what a 72-hour prototype would reveal.*
